...
class DistributedElevatorFSM(DistributedObject.DistributedObject, FSM):
    ...
    def finishSetup(self):
        self.isSetup = 1
        if self._DistributedElevatorFSM__preSetupState:
            self.request(self._DistributedElevatorFSM__preSetupState, 0)
            self._DistributedElevatorFSM__preSetupState = None
    ...
    def setState(self, state, timestamp):
        if self.isSetup:
            ...
            self._DistributedElevatorFSM__preSetupState = state
    ...
    def enterClosing(self, ts):
        ...
        if self.closeDoors.isPlaying() and self.lastState == 'closed' or self.openDoors.isPlaying():
    ...
    def onDoorCloseFinish(self):
        for avId in self.boardedAvIds.keys():
            ...
            av.getParent().compareTo(self.getElevatorModel()) == 0
        

    
    def enterClosed(self, ts):
        self._DistributedElevatorFSM__doorsClosed(self.getZoneId())
    ...
    def _DistributedElevatorFSM__doorsClosed(self, zoneId):
        if self.localToonOnBoard:
